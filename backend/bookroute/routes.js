...
router.post('/', async (req,res)=>{
    ...
    const regisbook= await new bookReg({author, bookname, year, price, description})
...
router.put('/:id',async(req,res)=>{
    const {author, bookname, year, price, description}= req.body;
    const result= await bookReg.updateOne({_id:req.params.id},{$set:{author, bookname, year, price, description}})
